...
def multiplication(tensor1, tensor2):
    ...
    m1.set_all(tensor1.detach().numpy())
    m2.set_all(tensor2.detach().numpy())
    
    result = Matrix.tensor_multiplication(m1, m2)
...
class AttentionModified(nn.Module):
    ...
    def forward(
        ...
        values = values.transpose(1, 2) # (bs, n_local_heads, cache_len + seqlen, head_dim)
        #scores = torch.matmul(xq, keys.transpose(2, 3)) / math.sqrt(self.head_dim)
        scores = multiplication(xq, keys.transpose(2, 3)) / math.sqrt(self.head_dim)   # Our function

        if mask is not None:
            scores = scores + mask  # (bs, n_local_heads, seqlen, cache_len + seqlen)
        scores = F.softmax(scores.float(), dim=-1).type_as(xq)
        #output = torch.matmul(scores, values)  # (bs, n_local_heads, seqlen, head_dim)
        output = multiplication(scores, values) # Our function
